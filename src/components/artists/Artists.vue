<template>
  <div class="max-w-md m-auto py-10">
    ...
    <h3 class="font-mono font-regular text-3xl mb-4">Add a new College</h3>
    <form action="" @submit.prevent="addArtist">
      <div class="mb-6">
        <input class="input"
          autofocus autocomplete="off"
          placeholder="Type an College name"
          v-model="newArtist.name" />
      </div>
      <input type="submit" value="Add College" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green hover:bg-green-dark block w-full py-4 text-white items-center justify-center" />
    </form>
  ...
  methods: {
    ...
    addArtist () {
      ...
      this.$http.secured.post('/api/v1/artists/', { artist: { name: this.newArtist.name } })
        ...
        .catch(error => this.setError(error, 'Cannot create College'))
    },
    removeArtist (artist) {
      this.$http.secured.delete(`/api/v1/artists/${artist.id}`)
        ...
        .catch(error => this.setError(error, 'Cannot delete College'))
    },
    editArtist (artist) {
    ...
    updateArtist (artist) {
      ...
      this.$http.secured.patch(`/api/v1/artists/${artist.id}`, { artist: { title: artist.name } })
        .catch(error => this.setError(error, 'Cannot update College'))
    }
  }
}
</script>
